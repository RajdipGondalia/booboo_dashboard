...
if($mode=="edit")
{
    $id = $important_note->id;
    $note = $important_note->note;
    ...
    <div class="container xl:px-10 md:px-10 sm:px-4 px-0 mx-auto flex flex-col xl:flex align">
        ...
        <div class="flex flex-col bg-red-100 xl:mx-10 md:mx-10 sm:mx-2 mx-0 rounded-lg">
            ...
            <!-- <div  class="flex lg:flex-row xl:flex xl:flex-row md:flex-row sm:flex-col flex-col xl:px-12 md:px-12 sm:px-12 px-0 w-full"> -->
                <form action="{{ route('store_important_note_data') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
                        ...
                        <div class="xl:w-2/5 md:3/5 sm:w-full w-full flex flex-col">
                            <label class="block mb-5 self-center w-full">
                                <span class="after:content-[''] after:ml-0.5 after:text-red-500 block text-left text-sm text-gray-500 font-medium ">
                                    Note<span class="text-red-700">*</span>
                                </span>
                                <textarea type="text" name="note" class=" h-28 bg-white mt-1 px-3 p-1 shadow-sm border-gray-500 border-none placeholder-white focus:outline-none  employee-input-card p-2 md:p-2 h-10 text-left rounded-lg self-start w-full flex flex-row focus:border-red-300 focus:ring-red-300 block w-full  rounded-3xl sm:text-sm focus:ring" placeholder="">{{$note}}</textarea>
                            </label>
                        </div>
                        <div class="mb-10 mt-10 self-center lg:self-center flex flex-row">
                            <input type="hidden" name="mode" value="{{$mode}}" >
                            <input type="hidden" name="important_note_id" value="{{$id}}" >
